...
let
  ...
  versionMappings = {
    ...
    dotnet =
      ...
      in
      lib.mapAttrs (version: pkgName: pkgsUnstable.dotnetCorePackages.${pkgName}) versionToPkgName;
  };
    ...
    ocaml = {
      packages = builtins.attrValues {
        inherit (pkgsUnstable)
          ...
          opam
          ;
        inherit (pkgsUnstable.ocamlPackages)
          ocaml-lsp
          ocamlformat
          ;
      };
    ...
    perl = {
      packages = builtins.attrValues {
        inherit (pkgsUnstable)
          perl
          ;
        inherit (pkgsUnstable.perlPackages)
          PerlLanguageServer
          PerlCritic
          PerlTidy
          ;
    ...
    php = {
      packages = builtins.attrValues {
        inherit (pkgsUnstable)
          ...
          intelephense
          ;
        inherit (pkgsUnstable.phpPackages)
          composer
          psalm
          phpstan
          ;
...
in
{
  options.hm.languages = lib.mapAttrs (
    ...
    lib.mkOption {
      default = { };
      description = lib.options.mdDoc "Manages the development environment for the ${capitalize name} language.";
      type =
          ...
          lib.types.submodule {
            options = {
              enable = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = lib.options.mdDoc "Enable the development environment and tools for ${capitalize name}.";
              };
              version = lib.mkOption {
                type = lib.types.enum (lib.attrNames def.versionMap);
                default = def.defaultVersion;
                description = lib.options.mdDoc ''
                  Select the version of the ${capitalize name} SDK to install.
              ...
              extraPackages = lib.mkOption {
                ...
                default = [ ];
                description = lib.options.mdDoc ''
                  A list of extra packages to install alongside the standard ${capitalize name} toolchain.
                '';
              };
            };
          }
